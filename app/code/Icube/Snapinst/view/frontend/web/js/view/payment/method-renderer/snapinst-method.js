define(
    [
        ...
        'Icube_Snapinst/js/action/set-payment-method',
    ...
    function (Component, $, quote, urlBuilder, storage, errorProcessor, customer, fullScreenLoader, setPaymentMethodAction, additionalValidators, url, messageList) {
        ...
        return Component.extend({
            defaults: {
                template: 'Icube_Snapinst/payment/snapinst'
            },
            redirectAfterPlaceOrder: false,
            /** Open VT-Snap */
            afterPlaceOrder: function () {
                var production = window.checkoutConfig.payment.snapinst.production;
                var client_key = window.checkoutConfig.payment.snapinst.clientkey;
                var merchant_id = window.checkoutConfig.payment.snapinst.merchantid;
                var mixpanel_key = window.checkoutConfig.payment.snapinst.mixpanelkey;
                ...
                //$.getScript(js, function(){
                    $.ajax({
                        ...
                        mixpanel.track(
                            'pg-pay', {
                              merchant_id: merchant_id,
                              plugin_name: "installment_dragon",
                              snap_token: data
                            }
                        );
                                ...
                                try{
                                    snap.pay(token, 
                                    {
                                        ...
                                        onSuccess: function(result){
                                            trackResult(data, merchant_id, 'installment_dragon', 'success', result);
                                        ...
                                        onPending: function(result){
                                            trackResult(data, merchant_id, 'installment_dragon', 'pending', result);
                                            messageList.addSuccessMessage({
                                                message: result.status_message
                                            });
                                            window.location.replace(url.build('checkout/onepage/success'));
                                        ...
                                        onError: function(result){
                                            trackResult(data, merchant_id , 'installment_dragon', 'error', result);
                                        ...
                                        onClose: function(){
                                            trackResult(data, merchant_id, 'installment_dragon', 'close');
                                            $.ajax({
