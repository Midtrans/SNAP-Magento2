    ...
    function (Component, $, quote, urlBuilder, storage, errorProcessor, customer, fullScreenLoader, setPaymentMethodAction, additionalValidators, url, messageList) {
        ...
        return Component.extend({
            ...
            afterPlaceOrder: function () {
                ...
                var merchant_id = window.checkoutConfig.payment.snapio.merchantid;
                var mixpanel_key = window.checkoutConfig.payment.snapio.mixpanelkey;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            mixpanel.track(
                              'pg-pay', {
                                merchant_id: merchant_id,
                                plugin_name: "installment_offline",
                                snap_token: data
                              }
                            );
                            ...
                            snap.pay(token, {
                                onSuccess: function(result){
                                    trackResult(data, merchant_id, 'installment_offline', 'success', result);
                                ...
                                onPending: function(result){
                                    trackResult(data, merchant_id, 'installment_offline', 'pending', result);
                                    messageList.addSuccessMessage({
                                        message: result.status_message
                                    });
                                    window.location.replace(url.build('checkout/onepage/success'));
                                ...
                                onError: function(result){
                                    trackResult(data, merchant_id, 'installment_offline', 'error', result);
